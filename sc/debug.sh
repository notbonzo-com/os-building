...
QEMU_ARGS="-S -gdb stdio -m 32 -no-reboot -no-shutdown -M smm=off"
...
case "$1" in
    "floppy")   QEMU_ARGS="${QEMU_ARGS} -drive format=raw,file=$PWD/$2,if=floppy"
    ;;
    "disk")     QEMU_ARGS="${QEMU_ARGS} -drive format=raw,file=$PWD/$2,if=ide"
    ;;
