...
{
    return MBR_ReadSectors(&g_MBRS[g_BootMBR], 0, 1, g_Data->BS.BootSectorBytes);
}

bool FAT_ReadFat(size_t lbaIndex)
{
    return MBR_ReadSectors(&g_MBRS[g_BootMBR], g_Data->BS.BootSector.ReservedSectors + lbaIndex, FAT_CACHE_SIZE, g_Data->FatCache);
}
...
FAT_File* FAT_OpenEntry(FAT_DirectoryEntry* entry)
{
    ...
    if (!MBR_ReadSectors(&g_MBRS[g_BootMBR], FAT_ClusterToLba(fd->CurrentCluster), 1, fd->Buffer))
    {
